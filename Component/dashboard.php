...
// 🔹 Hitung total UMKM dari table `umkm_account`
$total_umkm = 0;
...
// 🔹 Ambil rata-rata ESG (optional: dari laporan_esg)
$avg_esg = 0;
...
// 🔹 Ambil data rata-rata skor per tanggal
$labels = [];
$data_scores = [];
...
// ambil daftar UMKM dari umkm_accounts
$umkmList = [];
...
<div class="sidebar">
  ...
  <a class="collapse-item" href="login.html">⚙️ Login</a>
</div>
...
const esgChart = new Chart(ctx, {
    type: 'line', // bisa diganti 'bar' kalau mau grafik batang
    data: {
        labels: <?= json_encode($labels) ?>,
        datasets: [{
            label: 'Rata-rata Skor ESG',
            data: <?= json_encode($data_scores) ?>,
            borderColor: 'rgba(75, 192, 192, 1)',
            backgroundColor: 'rgba(75, 192, 192, 0.2)',
            borderWidth: 2,
            tension: 0.3, // buat garis agak mulus
            fill: true,
    ...
    options: {
        ...
        scales: {
            y: {
                beginAtZero: true,
                max: 100, // karena skor ESG biasanya 0–100
                title: {
                    display: true,
                    text: 'Average Score'
                }
  ...
  fetch(`get_report.php?umkm_id=${umkmId}`)
    ...
    .then(data => {
      ...
      `;
      document.getElementById("reportScore").innerText = data.avg_score;
      ...
      document.getElementById("reportFooter").innerHTML = `Integrasi Maqasid Syariah: 
        <span style="color:green;">${data.maqasid_syariah ?? '-'}</span>`;
    });
}
